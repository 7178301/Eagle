 ...
 * <p/>
 */
public final class PositionMetricDisplacement extends Position {
    public PositionMetricDisplacement(double latitude, double longitude, double altitude, Angle yaw) {
        super(latitude, longitude, altitude, yaw);
    }

    public PositionMetricDisplacement(double latitude, double longitude, double altitude, Angle roll, Angle pitch, Angle yaw) {
        super(latitude, longitude, altitude, roll, pitch, yaw);
    }

    public PositionMetricDisplacement(PositionMetric positionMetric) {
        super(positionMetric);
    }

    @Override
    public PositionMetricDisplacement add(PositionMetricDisplacement position) {
        return new PositionMetricDisplacement(latitude + position.getLatitude(),
    ...
    public Position copy() {
        return new PositionMetricDisplacement(latitude, longitude, altitude, roll.copy(), pitch.copy(), yaw.copy());
    }

    @Override
    public boolean equals(Object obj) {
        if (!(obj instanceof PositionMetricDisplacement))
            return false;
        if (obj == this)
            return true;

        PositionMetricDisplacement position = (PositionMetricDisplacement) obj;
