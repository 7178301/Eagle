...
public class AndroidBearing extends AdaptorBearing implements SensorEventListener {
    ...
    class calculateFusedOrientationTask extends TimerTask {
        public void run() {
            ...
            for (int i = 0; i < 3; i++) {
                bearingData[i] = fusedOrientation[i] * 360 / (2 * (float) Math.PI);
            }

            if (bearingData[YAW] < 0)
                bearingData[YAW] = 180 + (180 - Math.abs(bearingData[YAW]));
    ...
    public boolean connectToSensor() {
        ...
        if (sensorManager.getDefaultSensor(Sensor.TYPE_MAGNETIC_FIELD) != null && sensorManager.getDefaultSensor(Sensor.TYPE_ACCELEROMETER) != null && sensorManager.getDefaultSensor(Sensor.TYPE_GYROSCOPE) != null) {
